...
public class addPol {
   ...
   // List<Integer> pol1 = new ArrayList<Integer>();
    ...
    public void parseAndAdd(String firstPol, String secondPol){
        ...
        addFirst(firstPolLength, secondPolLength);
        addSecond(firstPolLength, secondPolLength);
    }
//  3x^2+2x^1+6x^0
    ...
    public void addFirst(int firstPolLength, int secondPolLength){
        try{
                ...
                while (j < firstPolLength) {
                    ...
                    while (k < secondPolLength) {
                        if(pol1[j].getDegree() == pol2[k].getDegree()){
                            polResult[polResultLength].setCoef(pol1[j].getCoef() + pol2[k].getCoef());
                        }
                        k++;
                    }
                    polResultLength++;
                ...
                while (j < secondPolLength) {
                    ...
                    while (k < firstPolLength) {
                        if(pol2[j].getDegree() == pol1[k].getDegree()){
                            polResult[polResultLength].setCoef(pol2[j].getCoef() + pol1[k].getCoef());
                        }
                        k++;
                    }
                    polResultLength++;
    ...
    public void addSecond(int firstPolLength, int secondPolLength){
        try {
            int polResultLength = 0;
            int i = 0;
            int j = 0;
            while((i < firstPolLength) || (j < secondPolLength)){
                if(pol1[i].getDegree() > pol2[j].getDegree()){
                    ...
                    polResult[polResultLength] = new Polynomial(pol2[j].getDegree(), pol2[j].getCoef() + pol1[i].getCoef());
                    j++;
                    i++;
                    polResultLength++;
                }
